tinymce.PluginManager.add("rdface", function (a, b) {
    ...
    a.addButton("rdfaceRun", {
        text: "Analyze",
        tooltip: "Refresh Semantic Content",
        icon: !1,
        onclick: function (v1) {
    ...
    $.getJSON(b + '/schema_creator/selection.json', function (data) {
        ...
        a.on("contextmenu", function (event) {
            ...
            // inline
            if (a.inline) {
                $(a.getDoc()).find(' .mce-floatpanel.mce-menu').hide();
            } else {
                // iframe
                $(' .mce-floatpanel.mce-menu').hide();
            }
            main_menu = [];
            ...
            while (parent_node.length) {
                if (parent_node.hasClass('r_entity')) {
                    ...
                    var entity_type = getTypeOfEntity(parent_node, $.cookie("annotationF"));
            ...
            while (parent_node.length) {
                ...
                if (parent_node.hasClass('r_prop') || parent_node.hasClass('r_entity')) {
                    main_menu.push({text: 'Delete', onclick: function () {
                            remove_annotation(del_node, $.cookie("annotationF"));
                            a.setContent($(a.getBody()).html());
                        }})
                    break;
                }
                parent_node = parent_node.parent();
            ...
            a.inline || (r = tinymce.DOM.getPos(a.getContentAreaContainer()), r.x += event.clientX, r.y += event.clientY), t.moveTo(r.x, r.y), a.on("remove", function () {
                t.remove(), t = null
            })
        });
